...
/**
 * Manual height update for vertical swiper direction, based on maximum .swiper-caption height
 * @this Swiper instance
 */
function swiperManualHeightUpdate () {
	var
		captions = this.el.querySelectorAll( '.swiper-slide-caption' ),
		heights = [],
		maxHeight;

	captions.forEach( function( caption ) {
		heights.push( caption.getBoundingClientRect().height );
	});
		...
		for ( var i = 0; i < plugins.swiper.length; i++ ) {
			var
				...
				options = {
					...
					on: {
						init: function() {
							// Change swiper height in vertical direction
							swiperManualHeightUpdate.call( this );
							this.on( 'resize', swiperManualHeightUpdate );
						}
					}
				};
	...
	if (plugins.slick.length) {
		for ( var i = 0; i < plugins.slick.length; i++ ) {
			...
			$slickItem.slick({
				...
				prevArrow: $slickItem.attr('data-custom-arrows') === 'true' ? $('.slick-prev[data-slick="' + $slickItem.attr('id') + '"]') : null,
				nextArrow: $slickItem.attr('data-custom-arrows') === 'true' ? $('.slick-next[data-slick="' + $slickItem.attr('id') + '"]') : null,
